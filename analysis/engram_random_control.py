"""
Random-weight control for the engram overlap probe.

E_j = W C_j pinv(C_tot) mixes the trained weights with INPUT statistics.
If a shape-matched random W reproduces the within-vs-between gap, the
'style knowledge is in the weights' reading is an artifact of input
separability. Run with the same cached covariances as the real probe.
"""
...
p.add_argument("--mode", choices=["random", "shuffled"], default="random")
...
if args.mode == "random":
    # shape-matched gaussian, scaled to the real weight's std
    weights = {n: torch.randn(w.shape, generator=g) * w.float().std()
               for n, w in weights.items()}
else:
    # permute entries of the real W: same marginal stats, structure destroyed
    weights = {n: w.float().flatten()[torch.randperm(w.numel(), generator=g)].reshape(w.shape)
               for n, w in weights.items()}
...
print(f"{len(styles)} styles, {len(layer_names)} layers, mode={args.mode}")
...
between = [0.5 * (ccos(si, sj) + ccos(sj, si))
           for i, si in enumerate(groups) for sj in groups[i+1:] if "neutral" not in (si, sj)]
summary = {
